...
// All mobile screens reused on desktop — full functionality preserved
...
// ─── ICONS ────────────────────────────────────────────────
function IconCart() {
  return <svg width="40" height="40" viewBox="0 0 40 40" fill="none"><rect x="1" y="1" width="38" height="38" rx="10" fill={T.yellow} stroke={T.navy} strokeWidth="2"/><path d="M9 11H12L15 24H28L30 16H13" stroke={T.navy} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"/><circle cx="17" cy="28" r="2.2" fill={T.navy}/><circle cx="26" cy="28" r="2.2" fill={T.navy}/></svg>;
}
function IconHandshake() {
  return <svg width="40" height="40" viewBox="0 0 40 40" fill="none"><rect x="1" y="1" width="38" height="38" rx="10" fill={T.yellow} stroke={T.navy} strokeWidth="2"/><path d="M8 22L13 26.5L18 23L22 26.5L29 22" stroke={T.navy} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"/><path d="M8 17L13 14H19L23 17H29" stroke={T.navy} strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"/><path d="M13 27V31M29 22V27" stroke={T.navy} strokeWidth="2.2" strokeLinecap="round"/></svg>;
}
function IconPackage() {
  return <svg width="40" height="40" viewBox="0 0 40 40" fill="none"><rect x="1" y="1" width="38" height="38" rx="10" fill={T.yellow} stroke={T.navy} strokeWidth="2"/><rect x="10" y="20" width="20" height="13" rx="2.5" fill="white" stroke={T.navy} strokeWidth="2"/><path d="M10 20L13.5 14H26.5L30 20" stroke={T.navy} strokeWidth="2" strokeLinejoin="round"/><path d="M17 20V14" stroke={T.navy} strokeWidth="2"/><circle cx="28" cy="16" r="5.5" fill={T.yellow} stroke={T.navy} strokeWidth="2"/><path d="M25.5 16L27.2 17.7L30.5 14.5" stroke={T.navy} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>;
}
function IconChat() {
  return <svg width="40" height="40" viewBox="0 0 40 40" fill="none"><rect x="1" y="1" width="38" height="38" rx="10" fill={T.yellow} stroke={T.navy} strokeWidth="2"/><rect x="8" y="10" width="17" height="13" rx="3.5" fill="white" stroke={T.navy} strokeWidth="2"/><path d="M11 27L9 31V27H7" stroke={T.navy} strokeWidth="2" strokeLinejoin="round"/><circle cx="12.5" cy="16.5" r="1.8" fill={T.navy}/><circle cx="16.5" cy="16.5" r="1.8" fill={T.navy}/><circle cx="20.5" cy="16.5" r="1.8" fill={T.navy}/><rect x="17" y="18" width="15" height="11" rx="3.5" fill={T.yellow} stroke={T.navy} strokeWidth="2"/><path d="M32 29V32L29 29" stroke={T.navy} strokeWidth="2" strokeLinejoin="round"/></svg>;
}
...
function DesktopNav({ screen, onNavigate, onNotif, onProfile, onSell }: {
  ...
  return (
    <nav style={{ position: "sticky", top: 0, zIndex: 100, background: "rgba(255,255,255,0.94)", backdropFilter: "blur(14px)", borderBottom: `1px solid ${T.border}`, boxShadow: T.s1 }}>
      <div style={{ maxWidth: MAX_W, margin: "0 auto", padding: PAD, display: "flex", alignItems: "center", height: "64px", gap: "28px" }}>
        <div onClick={() => onNavigate("Home")} style={{ display: "flex", alignItems: "center", gap: "8px", cursor: "pointer", flexShrink: 0 }}>
          <LogoBall />
          <div>
            <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: "18px", fontWeight: 800, color: T.navy, lineHeight: 1 }}>Rare<span style={{ color: T.coral }}>Nook</span></div>
        ...
        <div style={{ display: "flex", gap: "2px", flex: 1 }}>
          {links.map(l => {
            const on = screen === l.screen || (screen === "CardDetail" && l.screen === "Marketplace") || (screen === "TradeOffer" && l.screen === "TradeHub");
            return (
        ...
        <div style={{ display: "flex", gap: "10px", alignItems: "center" }}>
          <div onClick={onSell} style={{ padding: "8px 18px", background: T.navy, borderRadius: "10px", fontFamily: "'Space Grotesk', sans-serif", fontSize: "13px", fontWeight: 700, color: T.yellow, cursor: "pointer", whiteSpace: "nowrap" as const }}>+ Sell a Card</div>
          <div onClick={onNotif} style={{ width: "36px", height: "36px", borderRadius: "50%", background: T.white, border: `1.5px solid ${T.border}`, display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer", position: "relative" }}>
            <svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M8 1C5.8 1 4 2.8 4 5V8L2 10V11H14V10L12 8V5C12 2.8 10.2 1 8 1Z" stroke={T.inkMid} strokeWidth="1.5" strokeLinejoin="round"/><path d="M6.5 12.5C6.5 13.3 7.2 14 8 14C8.8 14 9.5 13.3 9.5 12.5" stroke={T.inkMid} strokeWidth="1.5" strokeLinecap="round"/></svg>
...
function DesktopHome({ onNavigate, onListing, onInfoPage }: {
  ...
  function handleCardClick(cardId: number) {
    ...
    // non-demo cards show a popup, not the detail page
  }
    ...
    <div style={{ maxWidth: MAX_W, margin: "0 auto", padding: `36px clamp(20px,3.5vw,48px)` }}>
      ...
      <div style={{ background: "linear-gradient(135deg,#FFD740 0%,#FFE878 55%,#FFF5C0 100%)", borderRadius: "20px", padding: "48px 56px", marginBottom: "36px", position: "relative", overflow: "hidden", boxShadow: T.s2 }}>
        ...
        <div style={{ maxWidth: "520px", position: "relative", zIndex: 1 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "14px" }}>
            <div style={{ background: T.navy, color: T.yellow, fontFamily: "'Space Grotesk', sans-serif", fontSize: "11px", fontWeight: 700, padding: "4px 12px", borderRadius: "50px" }}>🇮🇳 India's First Collector Marketplace</div>
          ...
          <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: "36px", fontWeight: 800, color: T.navy, lineHeight: 1.15, marginBottom: "12px" }}>Buy, Sell &amp; Trade<br/>Cards Safely in India</div>
          <div style={{ fontFamily: "'Inter', sans-serif", fontSize: "15px", color: "rgba(28,35,64,0.6)", lineHeight: 1.7, marginBottom: "24px" }}>Real ₹ pricing · Escrow-protected trades · Verified collectors across India</div>
          <div style={{ display: "flex", gap: "10px" }}>
            <div onClick={() => onNavigate("Marketplace")} style={{ padding: "12px 26px", background: T.navy, borderRadius: "12px", fontFamily: "'Space Grotesk', sans-serif", fontSize: "14px", fontWeight: 700, color: T.yellow, cursor: "pointer", boxShadow: "3px 4px 0 rgba(28,35,64,0.18)", transition: "transform 0.15s" }} onMouseEnter={e => (e.currentTarget as HTMLElement).style.transform = "translateY(-2px)"} onMouseLeave={e => (e.currentTarget as HTMLElement).style.transform = "translateY(0)"}>Browse Cards</div>
            <div onClick={() => onListing("sell")} style={{ padding: "12px 26px", background: "rgba(255,255,255,0.6)", borderRadius: "12px", fontFamily: "'Space Grotesk', sans-serif", fontSize: "14px", fontWeight: 600, color: T.navy, cursor: "pointer", border: "1.5px solid rgba(28,35,64,0.18)" }}>Sell a Card</div>
            <div onClick={() => onNavigate("TradeHub")} style={{ padding: "12px 26px", background: "transparent", borderRadius: "12px", fontFamily: "'Space Grotesk', sans-serif", fontSize: "14px", fontWeight: 600, color: T.navy, cursor: "pointer", border: "1.5px solid rgba(28,35,64,0.25)" }}>Start Trading</div>
          </div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "14px", marginBottom: "44px" }}>
        {[
          ...
          <div key={f.label} onClick={() => f.screen ? onNavigate(f.screen) : onInfoPage("about")}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "28px", marginBottom: "44px" }}>
        <div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px" }}>
            {tcgCards.slice(0, 4).map(card => (
              <div key={card.id} onClick={() => { if (card.real) onNavigate("CardDetail"); else onNavigate("Marketplace"); }}
                ...
                <div style={{ padding: "10px 11px 12px" }}>
                  ...
                  <div style={{ display: "inline-block", fontSize: "10px", fontWeight: 700, padding: "2px 8px", borderRadius: "50px", border: "1px solid rgba(0,0,0,0.08)", ...condStyle[card.condType], marginBottom: "5px" }}>{card.cond}</div>
        ...
        <div style={{ display: "flex", flexDirection: "column" as const, gap: "22px" }}>
          <div>
            ...
            <div style={{ background: T.white, borderRadius: "16px", overflow: "hidden", boxShadow: T.s1, border: `1.5px solid ${T.border}` }}>
              {MOVERS.map((m, i) => (
                <div key={m.id} style={{ display: "flex", alignItems: "center", gap: "13px", padding: "12px 15px", borderTop: i > 0 ? `1px solid rgba(28,35,64,0.06)` : "none", cursor: "pointer", transition: "background 0.15s" }} onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = T.skySoft} onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = "transparent"}>
                  <div style={{ width: "42px", height: "42px", borderRadius: "11px", background: m.bg, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><CardArt artKey={m.art as ArtKey} size={30} /></div>
                  <div style={{ flex: 1 }}>
        ...
        {tcgCards.map(card => (
          <div key={card.id} onClick={() => { if (card.real) onNavigate("CardDetail"); else onNavigate("Marketplace"); }}
            ...
            <div style={{ padding: "11px 12px 13px" }}>
              ...
              <div style={{ display: "inline-block", fontSize: "10px", fontWeight: 700, padding: "2px 9px", borderRadius: "50px", border: "1px solid rgba(0,0,0,0.08)", ...condStyle[card.condType], marginBottom: "6px" }}>{card.cond}</div>
              <div style={{ fontFamily: "'Inter', sans-serif", fontSize: "9px", color: T.inkSoft, fontWeight: 600, textTransform: "uppercase" as const, letterSpacing: "0.04em" }}>Market Value</div>
      ...
      <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: "24px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap" as const, gap: "14px" }}>
        ...
        <div style={{ display: "flex", gap: "6px" }}>
          {(["about","help","contact"] as const).map(p => (
            <div key={p} onClick={() => onInfoPage(p)} style={{ padding: "6px 14px", background: T.white, borderRadius: "50px", border: `1.5px solid ${T.border}`, fontFamily: "'Inter', sans-serif", fontSize: "11px", fontWeight: 600, color: T.inkMid, cursor: "pointer" }}>
...
// ─── DESKTOP MARKETPLACE (working search + card.real check) ──
  ...
  return (
    <div style={{ maxWidth: MAX_W, margin: "0 auto", padding: `36px clamp(20px,3.5vw,48px)` }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "22px", gap: "16px", flexWrap: "wrap" as const }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "8px", background: T.white, border: `1.5px solid ${T.border}`, borderRadius: "14px", padding: "9px 14px", boxShadow: T.s1, width: "280px" }}>
          ...
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search cards, sets, grades..." style={{ border: "none", outline: "none", fontFamily: "'Inter', sans-serif", fontSize: "13px", color: T.inkMid, background: "transparent", width: "100%" }} />
          {query && <div onClick={() => setQuery("")} style={{ cursor: "pointer", color: T.inkGhost, fontSize: "16px", lineHeight: 1, flexShrink: 0 }}>×</div>}
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: "8px", marginBottom: "26px", alignItems: "center", flexWrap: "wrap" as const }}>
        {cats.map(cat => {
          ...
          return <div key={cat} onClick={() => handleCat(cat)} style={{ padding: "7px 18px", borderRadius: "50px", fontFamily: "'Space Grotesk', sans-serif", fontSize: "13px", fontWeight: 700, cursor: "pointer", background: isOn ? T.navy : T.white, color: isOn ? T.yellow : T.inkMid, border: `1.5px solid ${isOn ? T.navy : T.border}`, transition: "all 0.15s" }}>{cat}</div>;
        })}
        <div style={{ marginLeft: "auto", fontFamily: "'Inter', sans-serif", fontSize: "13px", fontWeight: 600, color: T.inkSoft }}>{displayCards.length} result{displayCards.length !== 1 ? "s" : ""}</div>
      </div>
      ...
      {displayCards.length > 0 ? (
          ...
          {displayCards.map(card => (
            <div key={card.id} onClick={() => handleCardClick(card.id)}
              ...
              <div style={{ padding: "12px 13px 14px" }}>
                ...
                <div style={{ display: "inline-block", fontSize: "10px", fontWeight: 700, padding: "3px 10px", borderRadius: "50px", border: "1px solid rgba(0,0,0,0.08)", ...condStyle[card.condType], marginBottom: "7px" }}>{card.cond}</div>
                <div style={{ fontFamily: "'Inter', sans-serif", fontSize: "9px", color: T.inkSoft, fontWeight: 600, textTransform: "uppercase" as const, letterSpacing: "0.04em" }}>Market Value</div>
                <div style={{ fontFamily: "'Space Grotesk', sans-serif", fontSize: "17px", fontWeight: 800, color: T.ink }}>{card.price}</div>
                <div style={{ marginTop: "10px", padding: "9px 0", background: T.navy, borderRadius: "10px", fontFamily: "'Space Grotesk', sans-serif", fontSize: "12px", fontWeight: 700, color: T.yellow, textAlign: "center" as const }}>
                  {card.real ? "View Listing" : "Coming Soon"}
                </div>
              </div>
            </div>
          ))}
        ...
        <div style={{ textAlign: "center" as const, padding: "60px 0" }}>
          ...
          <div style={{ fontFamily: "'Inter', sans-serif", fontSize: "13px", color: T.inkSoft }}>Try searching for a card name, set, or condition</div>
...
export default function DesktopLayout({ screen, onNavigate }: {
  ...
  // Screens that get full desktop layout
  const isFullDesktop = screen === "Home" || screen === "Marketplace";
  ...
  return (
    <div style={{ minHeight: "100vh", background: T.sky, fontFamily: "'Inter', sans-serif" }}>
      ...
      <main>
        ...
        {/* Functional mobile screens centered in desktop — all interactions preserved */}
        {screen === "CardDetail"  && <DesktopScreenWrapper><CardDetailScreen  onNavigate={onNavigate} /></DesktopScreenWrapper>}
        {screen === "TradeHub"    && <DesktopScreenWrapper><TradeHubScreen    onNavigate={onNavigate} /></DesktopScreenWrapper>}
        {screen === "TradeOffer"  && <DesktopScreenWrapper><TradeOfferScreen  onNavigate={onNavigate} /></DesktopScreenWrapper>}
        {screen === "Community"   && <DesktopScreenWrapper><CommunityScreen   onNavigate={onNavigate} /></DesktopScreenWrapper>}
        {screen === "Profile"     && <DesktopScreenWrapper><ProfileScreen     onNavigate={onNavigate} /></DesktopScreenWrapper>}
      </main>
    </div>
  );
}
